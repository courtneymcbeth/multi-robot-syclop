    ...
    int refinement_level)
{
    ...
    // Step 1: Create decomposition covering ALL cells in expanded region
    // Each cell will be refined to (subdivision_factor x subdivision_factor) sub-cells
    oc::DecompositionPtr local_decomp;

    {
        ScopedFunctionTimer decomp_timer(timing_log_, "refineExpandedRegion.createDecomposition");
    ...
    // Step 2: Extract replanning bounds for the expanded region
    ...
    {
        ScopedFunctionTimer mapf_timer(timing_log_, "refineExpandedRegion.MAPF");
        auto mapf_solver = createMAPFSolver(
            config_.mapf_config.method,
            config_.mapf_config.region_capacity,
            config_.planning_time_limit);
    ...
    {
        ScopedFunctionTimer integrate_timer(timing_log_, "refineExpandedRegion.integratePaths");
        for (size_t i = 0; i < replan_robot_indices.size(); ++i) {
            if (replan_results[i].path != nullptr) {
                std::vector<size_t> single_robot = {replan_robot_indices[i]};
                std::vector<mr_syclop::GuidedPlanningResult> single_result = {replan_results[i]};
    ...
    {
        ScopedFunctionTimer recheck_timer(timing_log_, "refineExpandedRegion.recheckCollisions");
    ...
    CollisionResolutionEntry& log_entry)
{
    ...
    {
        ScopedFunctionTimer plan_timer(timing_log_, "localComposite.compositePlanning");
    ...
    if (result.solved && result.individual_paths.size() == 2) {
        ...
        {
            ScopedFunctionTimer integrate_timer(timing_log_, "localComposite.integratePaths");
            integrateRefinedPaths(robot_indices, local_results, update_info_1, update_info_2);
        }
        {
            ScopedFunctionTimer recheck_timer(timing_log_, "localComposite.recheckCollisions");
            recheckCollisionsFromTimestep(getRecheckStartTimestep(collision));
        }
                                                             ...
                                                             CollisionResolutionEntry& log_entry)
{
    ...
    for (int attempt = 1; attempt <= max_attempts; ++attempt) {
        ...
        PlanningResult result;
        {
            ScopedFunctionTimer plan_timer(timing_log_, "fullComposite.compositePlanning");
            result = useCompositePlanner(
                all_robot_indices,
                all_starts,
                all_goals,
        ...
        if (result.solved && result.individual_paths.size() == robots_.size()) {
            ...
            {
                ScopedFunctionTimer integrate_timer(timing_log_, "fullComposite.integratePaths");
                for (size_t i = 0; i < robots_.size(); ++i) {
                    mr_syclop::GuidedPlanningResult guided_result;
                    guided_result.success = true;
                    guided_result.path = result.individual_paths[i];
            ...
            {
                ScopedFunctionTimer recheck_timer(timing_log_, "fullComposite.recheckCollisions");
